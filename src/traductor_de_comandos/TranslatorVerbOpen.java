...
public class TranslatorVerbOpen implements CommandTranslator {

	private final static String[] VERBO_ABRIR = { " abrir ", " desbloquear " };

	private CommandTranslator next = null;

	public TranslatorVerbOpen(CommandTranslator next) {
		this.next = next;
	}
	...
	public void translateCommand(String command, Action action, Game game) {
		...
		int i = 0;
		while (!translated && i < VERBO_ABRIR.length) {

			if (command.contains(VERBO_ABRIR[i])) {
				translated = true;
